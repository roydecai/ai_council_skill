...
```bash
kimi -p "你正在参加 AI Council 第 N 轮讨论。请先回应其他 agent 的关键观点，再给出你的更新判断。输出必须包含：1. 观点 2. 思考逻辑 3. 支持性文件或依据" -S <session-id> --final-message-only | ~/.openclaw/scripts/clean-ansi.sh
```
...
3. `session` 机制用 `-S <id>` 实现多轮共享上下文，`-C` 续接上次
4. 带模型参数 `-m kimi-coding/k2p5` 时可能报 `LLM not set` 错误（已遇到一次）

解决方案：

- 使用 `-S <session-id>` 指定会议 session ID，每轮用同一个 id
...
```bash
# 1. 启动 Codex PTY 会话
exec(pty: true, background: true, command: "/Applications/Codex.app/Contents/Resources/codex -m gpt-5.4 --full-auto --dangerously-bypass-approvals-and-sandbox -y", workdir: "/tmp/ai-council-<meeting-id>")
...
10. **`--dangerously-bypass-approvals-and-sandbox` 与 `--full-auto` 可以同时使用**。MVP 实测已同时使用且未报错；后续文档统一按“可并用”处理，不再保留“会冲突”的旧说法
11. **`--reasoning-effort` 参数在顶层 CLI 不存在**，已实测报错 `unexpected argument '--reasoning-effort'`，不要使用
12. **依赖 ClashX 代理**（`http://127.0.0.1:7890`），通过 `~/.zshenv` 继承。⚠️ 这是当前环境特定配置，部署到其他 Mac/Linux 环境时需确认代理可用或移除
13. exec 默认 timeout 不够（OpenClaw 默认可能 60s），复杂任务需要 300s+；联网搜索密集型议题需要 600s+
14. 响应较慢，可能 2-5 分钟才有完整输出
15. **PTY 管道模式限制**：`codex | clean-ansi.sh` 会让 Codex 检测到非终端而报错 `stdout is not a terminal`，ANSI 清理只能在皮皮侧处理，不能用管道

解决方案：

- 工作目录：每次会议用 `mktemp -d && git init` 创建临时工作区
- 写文件：需要可写工作区时使用 `--dangerously-bypass-approvals-and-sandbox`
- 授权策略：会议期间默认使用 `--full-auto`；在当前 MVP 方案中可与 `--dangerously-bypass-approvals-and-sandbox` 同时开启
- 长对话：PTY 模式 + send-keys 逐条发送，不是 exec 一次性
- 输出清理：不要在 Codex 启动命令后直接接 `| ~/.openclaw/scripts/clean-ansi.sh`。正确做法是先采集 PTY 原始输出，再由主持人在采集层或归档前统一清理
- 代理：确认 `~/.zshenv` 中配置了 `http_proxy` / `https_proxy`
